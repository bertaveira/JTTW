...
void boxesoverlapLauncher(
    const int num_a, const float * boxes_a,
...
void nmsLauncher(
    const float * boxes, unsigned long long * mask, int boxes_num,
...
int boxes_overlap_bev_gpu(
    at::Tensor boxes_a, at::Tensor boxes_b,
    at::Tensor ans_overlap)
{
    ...
    boxesoverlapLauncher(
        num_a, boxes_a_data, num_b, boxes_b_data,
        ans_overlap_data);
...
int nms_gpu(
    at::Tensor boxes, at::Tensor keep,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        "boxes_overlap_bev_gpu", &boxes_overlap_bev_gpu,
